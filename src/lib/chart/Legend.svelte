...
<ul>
	...
	<li style="color: {textColor}">
		<svg width="20" height="30" class="bullet">
			<rect  width="20" height="30" style="fill:{colors[i]};">
			</rect>
		</svg>
		{item[xKey]} {toPerc(item[yKey])}
		{#if zKey == "prev"}
		<span class="" style="color: #3878C5">(2011 {changeStrLegend(Math.round(item[zKey]), '%)')}</span>
		{:else if zKey}
		<span class="" style="color: #3878C5">(NI {changeStrLegend(Math.round(item[zKey]), '%)')}</span>
		{/if}
	</li> 
	<br>
	{/each}
		...
		<tr>
			...
			<td class="cell-right">
				...
				{#if zKey == "prev"}
					<span class="" style="color: #3878C5">(2011 {changeStrLegend(Math.round(item[zKey]), '%)')}</span>
				{:else if zKey}
					<span class="" style="color: #3878C5">(NI {toPerc(item[zKey])})</span>
				{/if}
